...
var btnReset;
var winningPlr;
/* check assets for Database Structure*/
function preload(){
    redPlrAnim = loadAnimation("assests/player1a.png","assests/player1b.png","assests/player1a.png");
    yellowPlrAnim = loadAnimation("assests/player2a.png","assests/player2b.png","assests/player2a.png");
}
...
function setup(){
  ...
  redPlr.addAnimation("redMoving",redPlrAnim);
  redPlr.scale = 0.5
  ...
  yellowPlr.addAnimation("yellowMoving",yellowPlrAnim);
  yellowPlr.scale = -0.5
...
function draw(){
    ...
    if(gameState == 0){
      ...
      if(keyDown("space")){
        ...
        if(plrTurn != undefined || plrTurn !=null){
          alert( plrTurn +"Player's Turn");
          updateGameState(1); //To Start Playing
        }       
      }
    }
    ...
    if(gameState === 1){
      if(plrTurn === "RED")
      {
        ...
        // Red Player Move in all directions
        ...
        // yellow player Move sideways only
        if(keyDown("W")){
      ...
      if(plrTurn === "YELLOW")
      {
        ...
        // Yellow Player Move in all directions
        if(keyDown("A")){
    ...
    if(gameState === 2){
      ...
      if(winningPlr!=undefined || winningPlr != null){
        ...
        fill(Math.round(random(20,70)))
        textSize(30);
        text(winningPlr+ " player Wins!!",width/2-200,100);
        pop();
      }
    }
